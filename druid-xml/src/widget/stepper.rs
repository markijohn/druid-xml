...
use druid::{widget::{Stepper, Axis}, Widget, EventCtx, Event, Env, LifeCycleCtx, LifeCycle, LayoutCtx, BoxConstraints, PaintCtx, UpdateCtx, Size};
use serde_json::{Value, Number};

use crate::qwidget::value::JSValue;
use super::theme;

pub struct DXStepper {
	val : f64,
    origin : Stepper
}
...
impl DXStepper {
	...
	pub fn with_step(mut self, val:f64) -> Self {
        self.origin = self.origin.with_step(val);
        self
    }

	pub fn with_wraparound(mut self, val:bool) -> Self {
        self.origin = self.origin.with_wraparound(val);
        self
    }
}


impl Widget<JSValue> for DXStepper {
