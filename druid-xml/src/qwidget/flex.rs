...
impl Axis {
	pub fn origin_axis(self) -> druid::widget::Axis {
		match self {
			Self::Horizontal => druid::widget::Axis::Horizontal,
			Self::Vertical => druid::widget::Axis::Vertical
		}
	}
...
impl Widget<JSValue> for Flex {
    ...
    fn debug_state(&self, data: &JSValue) -> DebugState {
    ...
    fn compute_max_intrinsic(
        ...
        let oaxis = self.direction.origin_axis();
        if oaxis != axis {
            ...
            for child in self.children.iter_mut() {
                match child {
                    Child::Fixed { widget, .. } => {
                        let new_bc = bc
                            .unbound_max(axis)
                            .shrink_max_to(oaxis, available_size_on_main_axis);
                        ...
                        let new_bc = new_bc.shrink_max_to(oaxis, size_on_main_axis);
